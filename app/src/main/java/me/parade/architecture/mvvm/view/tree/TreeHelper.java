package me.parade.architecture.mvvm.view.tree;
    ...
    private static List<Node> convetData2Node(List<Node> nodes) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            for (int j = i + 1; j < nodes.size(); j++) {
                Node m = nodes.get(j);
                //不是人员 且n时父节点,使用kotlin 和gson实现深度复制时mgson会把对象转成linkedTreeMap
                if (/**((OrganizeTree) n.getBean()).getType().equals("org") && */ m.getpId().equals(n.getId())) {
                    n.getChildren().add(m);
                    m.setParent(n);
                } else if (/**((OrganizeTree) m.getBean()).getType().equals("org") &&*/ m.getId().equals(n.getpId())) {
                    m.getChildren().add(n);
                    n.setParent(m);
                }
            }
