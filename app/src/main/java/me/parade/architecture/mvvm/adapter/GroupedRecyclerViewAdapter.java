...
import me.parade.architecture.mvvm.data.bean.GroupStructure;
 ...
 *
 *用户展开收缩的列表时，此基础adapter会把你的数据解构转成GroupStructure
 */
public abstract class GroupedRecyclerViewAdapter
    ...
    //保存分组列表的组结构，在structureChanged里添加数据
    ...
    public void notifyChildRangeInserted(int groupPosition, int childPosition, int count) {
        if (groupPosition < mStructures.size()) {
            ...
            index += Math.min(childPosition, structure.getChildrenCount());
            if (count > 0) {
